...
標準は日本の歌本・教則本の形です：横向き（1弦が上・ナットが左・位置のフレット番号は下）、黒い丸、丸の中は指（人・中・薬・小・親）。セーハは帯1本で描き、指の文字は一番低い弦の側に1つだけ書きます。

```js
el.innerHTML = DS.chordbox.svg('C');                           // コード名から（いちばん上の候補）
el.innerHTML = DS.chordbox.svg('x32010', {chord:'C'});         // 押さえ方を直接
el.innerHTML = DS.chordbox.svg('2x0232', {fingers:'Tx0132', chord:'D/F#'});   // 親指は T
el.innerHTML = DS.chordbox.svg('F', {orient:'v', fingerStyle:'num'});         // 海外のコードブックの形・数字
...
- 指は `1`＝人差し指…`4`＝小指、`T`（または `親`）＝親指。省くと自動で振ります（自動では親指を使いません）
- `DS.chordbox.fingers(押さえ)` で指だけを得られます

### 設定
| svg の設定 | 中身 |
|---|---|
| `orient` | `'h'`＝横・歌本の形（初期）／`'v'`＝縦・海外のコードブックの形（位置は左に「3fr」） |
| `label` | 丸の中：`'finger'`＝指（初期）／`'none'`＝何も書かない／`'note'`＝音名／`'deg'`＝度数 |
| `fingerStyle` | `'kanji'`＝人・中・薬・小・親（初期）／`'num'`＝1〜4・親指は T／`'num-oya'`＝1〜4・親指は「親」 |
| `look` | `'book'`＝黒（初期）／`'paper'`＝白い紙＋度数の色／`'ebony'`＝指板そのまま |
| `chord` | コード名。音名の綴りと度数に使う（省くと音から当てる） |
| `title` | 上にコード名（コードが分かる時は初期で出す）。`false` で出さない、文字を渡せばその文字 |
| `names` | `true` で各弦の音名を図の外に書く（初期は書かない） |
| `rows` | 描くフレットの数（初期 4。はみ出す時は自動で広げる） |
| `base` | 一番左（縦なら上）のフレット。省くと自動（4フレットまでに収まればナットを描く） |
| `scale` | 大きさ（初期 1.3） |
...
### 押さえ方の出し方
1. 内蔵の表：開放弦を使う定番の形、親指で6弦を押さえる形（D/F♯・F・FM7）、6弦・5弦・4弦ルートの動かせる形（メジャー・マイナー・7・M7・m7・m7(♭5)・dim7・aug・sus4・7sus4・6・m6・9・パワーコード）
...
3. 並べ順は「表の形 → 低いポジション → 押さえやすさ」。同じ位置なら親指を使わない形が先

- 足したテンション（`C7(♭9,13)` など）・調弦を変えた時は、表を使わず探す方だけで出します。分数コードは、表にある親指の形のほかは探す方で出します
